var dataproject = {
    ...
    share: function() {
        $(".fa-twitter-square").on("click", function() {
            var tweet = "Former Baltimore County superintendent Dallas Dance traveled extensively. View his travel logs.";
        ...
        $(".fa-facebook-square").on("click", function() {
            var picture = "http://data.baltimoresun.com/news/dance-travel/images/thumb.png";
            var title = "Visualizing Dallas Dance's trips in 2016";
            var description = "Former Baltimore County superintendent Dallas Dance traveled extensively. View his travel logs.";
            var url = "http://data.baltimoresun.com/news/dance-travel/";
    ...
    calendar: function() {
        ...
        var tooltipline0 = tooltip.append("p").attr("class", "tooltip__text tooltip__text--date");
        var tooltipline1 = tooltip.append("p").attr("class", "tooltip__text");
        var tooltipline2 = tooltip.append("p").attr("class", "tooltip__text");
        var tooltipline3 = tooltip.append("p").attr("class", "tooltip__text");
        var ttTempHR = tooltip.append("hr").attr("class", "tooltip__text--temp");
        var ttTemp0 = tooltip.append("p").attr("class", "tooltip__text tooltip__text--date tooltip__text--temp").text(calEvents[38][0] + "/" + calEvents[38][1] + " - " + calEvents[38][0] + "/" + (calEvents[38][1] + calEvents[38][2]));
        var ttTemp1 = tooltip.append("p").attr("class", "tooltip__text tooltip__text--temp").text(calEvents[38][3] + ", " + calEvents[38][4]);
        var ttTemp2 = tooltip.append("p").attr("class", "tooltip__text tooltip__text--temp").html("<br><span>Event:</span> " + calEvents[38][6]);
        var ttTemp3 = tooltip.append("p").attr("class", "tooltip__text tooltip__text--temp").html("<span>Cost:</span> " + calEvents[38][5]);
        var ttTemp = d3.selectAll(".tooltip__text--temp").style("display", "none");
        var dataID, currData;
